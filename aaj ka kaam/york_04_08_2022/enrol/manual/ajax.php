...
//EDIT FROM HERE
$uid = $USER->id;
$select_admin = $DB->get_records_sql("select * from {school_user} where userid='$uid'");  //selecting SCHOOL ADMIN
$select_admin1 =count($select_admin);
if($select_admin1>0){

    $userids1 = optional_param_array('userlist', [], PARAM_SEQUENCE);
    $userid1 = optional_param('userid', 0, PARAM_INT);
    if ($userid1) {
        $userids1[] = $userid1;
    }
    if ($userids1) {
        foreach ($userids1 as $userid1) {
            $users1[] = $DB->get_record('user', array('id' => $userid1), '*', MUST_EXIST);
        }
    }
    foreach($users1 as $u1){
        ...
        $minus=1;
        $uid1 = $user->id;
        $sc_id = $DB->get_record_sql("select * from {school_createduser} where userid='$uid'");
        $schoolid = $sc_id->schoolid;
        $lis_count = $DB->get_record_sql("select * from {license_count} where schoolid=$schoolid and courseid='$id'");
        $useddata = $lis_count->used;
        $total = $lis_count->license_count;
        $url = $CFG->wwwroot.'/';
        
        //  LICENSE EXPIRY FROM HERE
                $get_course = $DB->get_record_sql("select * from {course} where id='$id'");
                ...
                if($date > $license_expiry->enddate){
                    ...
                    $outcome->error = 'All Licenses are Used';
                    die;
                }
        ...
        $DB->execute("update {license_count} set used=$useddata where schoolid='$schoolid' and courseid='$id'");
        // $plugin->enrol_user($instance, $user->id, $roleid, $timestart, $timeend, null, $recovergrades);

        //switch cases from here
        switch ($action) {
            case 'enrol':
