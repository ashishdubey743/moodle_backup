...
class course_enrolment_manager {
    ...
    public function get_potential_users($enrolid, $search = '', $searchanywhere = false, $page = 0, $perpage = 25,
        ...
        $id = $USER->id;
        $school_user = $DB->get_records_sql("select * from {school_createduser} where userid = '$id'");
        $school_user1 = count($school_user);
        if($school_user1>0){
            // $sql = " from {user} u left join {school_createduser} scu on u.id = scu.userid where scu.userid in (select scu.userid from {school_createduser} scu left join {school_user} su on scu.schoolid = su.schoolid where scu.userid = 176)";
            $sql = " from {user} u $joins LEFT JOIN {user_enrolments} ue ON (ue.userid = u.id AND ue.enrolid = :enrolid) left join {school_createduser} scu on scu.userid = u.id where $wherecondition AND ue.id IS NULL and scu.userid in (select scu.userid from {school_createduser} scu where scu.schoolid in ( select scu.schoolid from {school_createduser} scu where scu.userid = '$id'))";
