...
class global_navigation extends navigation_node {
    ...
    public function add_course_essentials($coursenode, stdClass $course) {
        ...
        //changes for user here
        global $USER,$DB,$SITE;
        $id = $USER->id;

        $coursecontext = context_course::instance($course->id);
        $roles = get_user_roles($coursecontext, $id, true);
        $role = key($roles);
        $roleid = $roles[$role]->roleid;
        $selectuser = $DB->get_records_sql("select * from {school_user} where userid = '$id'");
        $selectuser1 = count($selectuser);
        if($selectuser1 > 0 || is_siteadmin() || $roleid==4){
