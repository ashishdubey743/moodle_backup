...
if ($data = data_submitted() and confirm_sesskey() and isset($data->action) and $data->action == 'save-settings') {
    // selecting data
    $select = $DB->get_records_sql("select * from {buttoncolor}");
    $colorexist = $select->id;
    $select1 = count($select);
    $scolor = $data->s_theme_moove_Buttoncolors;
    if($select1>0){
        $DB->execute("update {buttoncolor} set scheme_color='$scolor'");
    }
    else{
        $recordtoinsert = new stdclass();
    $recordtoinsert->scheme_color = $data->s_theme_moove_Buttoncolors;
    
    $DB->insert_record('buttoncolor',$recordtoinsert);
    }
